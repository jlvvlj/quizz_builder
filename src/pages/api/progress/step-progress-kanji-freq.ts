...
// Step progress for the frequency kanji quiz. Mirrors /api/progress/step-progress
// over the kanji pronunciation-item rank window + kanji_quiz_item_progress.
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        // Resolve the same frequency column the deck uses (composed order by
        // default), so the progress window matches the kanji actually practiced —
        // otherwise the average is taken over a different 100-kanji set.
        const col = await resolveFrequencyColumn(req);
        const itemIds = await kanjiIdsForWindow(supabase, section as string, step as string, col);
        if (itemIds.length === 0) {
        ...
        console.error('Error calculating kanji-freq step progress:', error);
