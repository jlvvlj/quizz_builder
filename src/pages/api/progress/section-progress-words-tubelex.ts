...
// Section progress for the TUBELEX words quiz. Mirrors section-progress-kanji-freq
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        // Whole-section window (no step).
        const wordIds = await wordIdsForTubelexWindow(supabase, section as string);
        if (wordIds.length === 0) {
        ...
        console.error('Error calculating words-tubelex section progress:', error);
