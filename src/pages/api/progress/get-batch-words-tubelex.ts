...
// Per-user, per-quiz-type progress for a batch of word ids. Mirrors
// get-batch-kanji-freq but over words_tubelex_progress (word_id). Returns a map
// keyed by word id; missing ids resolve to the zeroed 'new' shape.
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        if (progressError) {
            console.error('Error fetching words-tubelex progress:', progressError);
        ...
        console.error('Error getting batch words-tubelex progress:', error);
