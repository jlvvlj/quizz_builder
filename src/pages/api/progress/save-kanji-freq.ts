...
// Saves frequency kanji quiz progress. Mirrors /api/progress/save (per-quiz-type
// progress_status + daily-activity bump) but writes kanji_quiz_item_progress keyed
// by (user_id, item_id, quiz_type). The request body reuses the words quiz's
// `wordDifficulties` accumulator verbatim — the keys are pronunciation item ids.
const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL!;
const supabaseServiceKey = process.env.SUPABASE_SERVICE_KEY!;
const supabase = createClient(supabaseUrl, supabaseServiceKey);

type Stage = 'parse_request' | 'kanji_progress_upsert' | 'activity_read' | 'activity_upsert';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        // Accept both `kanjiDifficulties` and `wordDifficulties` so the shared
        // quiz engine can post its existing accumulator unchanged.
        const difficulties = req.body.kanjiDifficulties ?? req.body.wordDifficulties;
        const { quizType: rawQuizType } = req.body;
        const quizType = typeof rawQuizType === 'string' && rawQuizType ? rawQuizType : 'multiple_choice';
        if (!difficulties) {
            return res.status(400).json({ error: 'kanjiDifficulties are required' });
        }
        ...
        const progressRecords = Object.entries(difficulties).map(([itemId, difficulty]: [string, any]) => {
        ...
        stage = 'kanji_progress_upsert';
        const { error: upsertError, data } = await supabase
            .from('kanji_quiz_item_progress')
            .upsert(progressRecords, { onConflict: 'user_id,item_id,quiz_type' })
            .select();
        if (upsertError) {
        ...
        console.error(`Error saving kanji-freq progress at stage="${failedStage}":`, error);
