...
// Per-user, per-quiz-type progress for a batch of kanji quiz item ids. Mirrors
// /api/progress/get-batch but over kanji_quiz_item_progress (item_id). Returns a
// map keyed by item id; missing ids resolve to the zeroed 'new' shape.
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const cardIdArray = cardIds.split(',').map(id => {
            ...
            if (isNaN(parsed)) throw new Error(`Invalid card ID: ${id}`);
            return parsed;
        });
        ...
        if (progressError) {
            console.error('Error fetching kanji-freq progress:', progressError);
        ...
        for (const cardId of cardIdArray) {
            const progress = progressData?.find(p => p.item_id === cardId);
            if (progress) {
                progressMap[cardId] = {
        ...
        console.error('Error getting batch kanji-freq progress:', error);
