...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        console.log('Kanji session init request received');
        ...
        console.error('Error initializing kanji session:', error);
        res.status(500).json({ error: 'Failed to initialize kanji session', details: error instanceof Error ? error.message : 'Unknown error' });
    }
} 
