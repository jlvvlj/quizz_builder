...
// Completion % for an anime episode = SAME calculation as a course
// section/step (calculateStepProgressFromRecords), but the word set is
// the episode's words10k ids from anime_word. READ ONLY — never writes
// user_progress.
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        const records: { word_id: number; progress_status: string | null; progress: number }[] = [];
        for (let i = 0; i < wordIds.length; i += 1000) {
            const slice = wordIds.slice(i, i + 1000);
            const { data, error } = await supabase
                .from('user_progress')
        ...
        console.error('anime/episode-progress error:', e);
        return res.status(500).json({ error: 'Failed to calculate episode progress' });
    }
}
