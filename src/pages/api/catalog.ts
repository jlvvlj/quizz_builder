...
export default async function handler(req:NextApiRequest,res:NextApiResponse) {
 ...
 try {
  ...
  const rows:{id:number;deck_id:string;section:string;step:string}[]=[];
  for(let from=0;;from+=1000){const {data,error}=await serverDb.from('words10k').select('id,deck_id,section,step').order('id').range(from,from+999);if(error)throw error;rows.push(...data);if(data.length<1000)break;}
  ...
  for(const row of rows){
   ...
   if(!step){step={key:row.step,id:Number(row.step.replace('step_','')),items:0};section.steps.push(step);}step.items++;
  }
